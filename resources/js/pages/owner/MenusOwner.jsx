...
import {
    ...
    Check, X, FlaskConical, Minus, Image, Calendar, Zap,
} from "lucide-react";
...
const JENIS = [
    { value: "Harian",     label: "Harian",     icon: Calendar, color: "#10b981", bg: "rgba(16,185,129,.12)", border: "rgba(16,185,129,.30)" },
    { value: "Insidentil", label: "Insidentil",  icon: Zap,      color: "#f59e0b", bg: "rgba(245,158,11,.12)",  border: "rgba(245,158,11,.30)"  },
];

const emptyForm = {
    name: "", description: "", category: "", price: "",
    min_pax: "", status: "active", jenis_catering: "Harian",
...
function JenisBadge({ jenis }) {
    const j = JENIS.find(j => j.value === jenis) || JENIS[0];
    const Icon = j.icon;
    return (
        <span style={{ display: "inline-flex", alignItems: "center", gap: "4px", fontSize: "11px", fontWeight: 600, padding: "3px 10px", borderRadius: "20px", background: j.bg, border: `0.5px solid ${j.border}`, color: j.color }}>
            <Icon size={10} strokeWidth={2.5} /> {j.label}
        </span>
    );
}
...
function FilterTabs({ active, onChange, counts }) {
    const tabs = [
        ...
        { value: "Harian",     label: "Harian",      count: counts.harian,     color: "#10b981" },
        { value: "Insidentil", label: "Insidentil",  count: counts.insidentil, color: "#f59e0b" },
    ];
    return (
            ...
            {tabs.map(t => {
                ...
                return (
                    <button key={t.value} onClick={() => onChange(t.value)} style={{
                        ...
                        border: isActive ? `0.5px solid ${color}50` : `0.5px solid ${C.border}`,
                        background: isActive ? `${color}15` : "transparent",
                        color: isActive ? color : C.muted,
                        fontFamily: C.font, fontSize: "12.5px", fontWeight: 600,
                        cursor: "pointer", display: "flex", alignItems: "center", gap: "6px",
                    }}>
                        {t.label}
                        <span style={{ fontSize: "10px", fontWeight: 700, padding: "1px 6px", borderRadius: "20px", background: isActive ? `${color}25` : C.card, color: isActive ? color : C.muted }}>
                            {t.count ?? 0}
                        </span>
                    </button>
                );
...
function StatCard({ label, value, icon: Icon, bar }) {
    return (
        <div style={{ background: C.surface, border: `0.5px solid ${C.border}`, borderRadius: "12px", padding: "18px 20px", position: "relative", overflow: "hidden", fontFamily: C.font }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>
                ...
                <div style={{ width: "38px", height: "38px", borderRadius: "9px", background: C.card, border: `0.5px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "center", color: C.muted }}>
...
function IngredientsPanel({ ingredients, setIngredients, stocks }) {
    const addRow    = () => setIngredients(prev => [...prev, { stock_id: "", qty_per_portion: "" }]);
    ...
    return (
        <div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "8px" }}>
                <div style={{ fontSize: "11px", fontWeight: 600, color: C.muted, textTransform: "uppercase", letterSpacing: ".6px" }}>Bahan per porsi</div>
                <button type="button" onClick={addRow} style={{ height: "26px", padding: "0 10px", borderRadius: "6px", border: "0.5px solid rgba(99,102,241,.35)", background: "rgba(99,102,241,.12)", color: "#a5b4fc", fontFamily: C.font, fontSize: "11px", fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: "4px" }}>
                    <Plus size={11} strokeWidth={2.5} /> Tambah bahan
                </button>
            </div>
            {ingredients.length === 0 ? (
                <div style={{ padding: "14px", borderRadius: "8px", background: "rgba(255,255,255,0.02)", border: `0.5px solid ${C.border}`, fontSize: "12px", color: C.muted, textAlign: "center" }}>
                    ...
                    {ingredients.map((row, i) => {
                        ...
                        return (
                            <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 90px 50px 28px", gap: "6px", alignItems: "center" }}>
                                <select style={selectStyle} value={row.stock_id} onChange={e => updateRow(i, "stock_id", e.target.value)}>
                                    <option value="">— pilih bahan —</option>
                                    {stocks.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                                </select>
                                <input style={{ ...inp, textAlign: "right" }} type="number" placeholder="0" min="0" step="0.001" value={row.qty_per_portion} onChange={e => updateRow(i, "qty_per_portion", e.target.value)} />
                                <div style={{ height: "40px", borderRadius: "8px", border: `0.5px solid ${C.border}`, background: "rgba(255,255,255,0.03)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "12px", color: C.muted, fontFamily: C.font }}>
                                    {found?.unit || "—"}
                                </div>
                                <button type="button" onClick={() => removeRow(i)} style={{ width: "28px", height: "28px", borderRadius: "6px", border: "0.5px solid rgba(239,68,68,.25)", background: "rgba(239,68,68,.08)", color: "#fca5a5", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
...
/* ── MenuModal — self-contained, punya state sendiri ── */
function MenuModal({ show, editId, initialForm, ingredients, setIngredients, stocks, onClose, onSubmit }) {
    // ✅ State form ada DI DALAM modal — tidak ada stale closure
    const [form, setForm] = useState(initialForm);

    // Sync ketika initialForm berubah (saat buka edit)
    useEffect(() => {
        setForm(initialForm);
    }, [initialForm]);
    ...
    const previewUrl = form.imageFile
        ...
        : form.image ? `/storage/${form.image}` : null;

    const handleLocalSubmit = (e) => {
        e.preventDefault();
        onSubmit(form); // kirim form terkini ke parent
    };
    ...
    return (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, padding: "20px" }}>
            <div style={{ width: "100%", maxWidth: "520px", background: C.surface, border: `0.5px solid ${C.borderMd}`, borderRadius: "14px", padding: "24px", fontFamily: C.font, maxHeight: "90vh", overflowY: "auto" }}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "20px" }}>
                    <div>
                        <div style={{ fontSize: "18px", fontWeight: 800, color: C.text, letterSpacing: "-.4px" }}>{editId ? "Edit menu" : "Tambah menu"}</div>
                        <div style={{ fontSize: "12.5px", color: C.muted, marginTop: "4px" }}>Isi detail menu dan bahan baku per porsi</div>
                    </div>
                ...
                <form onSubmit={handleLocalSubmit}>
                    ...
                    {/* Jenis Catering toggle */}
                    <FieldLabel mt={false}>Jenis Catering</FieldLabel>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px", marginBottom: "4px" }}>
                        {JENIS.map(j => {
                            const Icon = j.icon;
                            const sel  = form.jenis_catering === j.value;
                            return (
                                <button key={j.value} type="button"
                                    onClick={() => setForm(prev => ({ ...prev, jenis_catering: j.value }))}
                                    style={{
                                        height: "44px", borderRadius: "9px",
                                        border: sel ? `1px solid ${j.border}` : `0.5px solid ${C.border}`,
                                        background: sel ? j.bg : "transparent",
                                        color: sel ? j.color : C.muted,
                                        fontFamily: C.font, fontSize: "13px", fontWeight: 700,
                                        cursor: "pointer", display: "flex", alignItems: "center",
                                        justifyContent: "center", gap: "7px",
                                    }}>
                                    <Icon size={14} strokeWidth={2} /> {j.label}
                    ...
                    {previewUrl && (
                        <div style={{ position: "relative", marginBottom: "8px" }}>
                            <img src={previewUrl} alt="preview" style={{ width: "100%", height: "140px", objectFit: "cover", borderRadius: "8px", border: `0.5px solid ${C.border}` }} />
                            <button type="button" onClick={() => setForm(prev => ({ ...prev, image: null, imageFile: null }))}
                    ...
                    <label style={{ display: "flex", alignItems: "center", gap: "8px", height: "40px", borderRadius: "8px", border: `0.5px dashed ${C.borderMd}`, background: C.card, padding: "0 12px", cursor: "pointer", fontSize: "13px", color: C.muted }}>
                        <Image size={14} strokeWidth={1.8} />
                        <span>{form.imageFile ? form.imageFile.name : "Pilih foto (jpg, png, webp, maks 2MB)"}</span>
                        <input type="file" accept="image/jpeg,image/png,image/webp" style={{ display: "none" }}
                            onChange={e => { const file = e.target.files[0] || null; setForm(prev => ({ ...prev, imageFile: file })); }} />
                    </label>
                    ...
                    <IngredientsPanel ingredients={ingredients} setIngredients={setIngredients} stocks={stocks} />
                    ...
                    <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px", marginTop: "22px", paddingTop: "16px", borderTop: `0.5px solid ${C.border}` }}>
                        ...
                        <button type="submit" style={{ height: "36px", padding: "0 18px", borderRadius: "8px", border: "none", background: "#6366f1", color: "#fff", fontFamily: C.font, fontSize: "13px", fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: "5px" }}>
                            <Check size={14} strokeWidth={2.5} /> {editId ? "Update" : "Simpan"}
                        </button>
                    </div>
                </form>
            </div>
...
export default function MenusOwner() {
    ...
    const fetchMenus = async () => {
        try {
            ...
            setMenus(res.data);
        } catch (err) { console.error("fetchMenus error:", err.response?.data || err); }
    };
    ...
    const fetchStocks = async () => {
        try {
            ...
            setStocks(res.data);
        } catch (err) { console.error("fetchStocks error:", err.response?.data || err); }
    };

    useEffect(() => { fetchMenus(); fetchStocks(); }, []);

    const filtered = filterJenis === "semua" ? menus : menus.filter(m => m.jenis_catering === filterJenis);
    const counts = {
        semua:      menus.length,
        harian:     menus.filter(m => m.jenis_catering === "Harian").length,
        insidentil: menus.filter(m => m.jenis_catering === "Insidentil").length,
    };

    // ✅ handleSubmit terima form dari modal langsung — tidak ada stale closure
    const handleSubmit = async (form) => {
        try {
            ...
            fd.append("jenis_catering", form.jenis_catering);
            ...
            const msg = err.response?.data?.message || JSON.stringify(err.response?.data) || "Terjadi kesalahan.";
            alert(msg);
        }
    };

    const closeModal = () => { setShowModal(false); setEditId(null); setIngredients([]); setInitialForm(emptyForm); };
    ...
    const handleEdit = async (item) => {
        const f = {
            ...
            jenis_catering: item.jenis_catering || "Harian",
        ...
        try {
            ...
            setIngredients(res.data.map(i => ({ stock_id: String(i.stock_id), qty_per_portion: i.qty_per_portion, unit: i.stock?.unit || "" })));
    ...
    const openCreate = () => { setEditId(null); setIngredients([]); setInitialForm(emptyForm); setShowModal(true); };
            ...
            <div style={{ fontFamily: C.font }}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "16px", marginBottom: "28px", flexWrap: "wrap" }}>
                    <div>
                        <div style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "11px", fontWeight: 600, color: C.muted, textTransform: "uppercase", letterSpacing: ".8px", marginBottom: "14px" }}>
                            <LayoutDashboard size={13} strokeWidth={2} /><span>Owner</span><span style={{ color: "#1E293B" }}>›</span><span>Menu</span>
                        </div>
                        <h1 style={{ fontSize: "28px", fontWeight: 800, color: C.text, letterSpacing: "-.8px", lineHeight: 1.1, margin: 0 }}>Menu catering</h1>
                        <p style={{ marginTop: "8px", fontSize: "13.5px", color: C.muted, lineHeight: "1.7" }}>Kelola menu, harga, dan resep bahan baku per porsi.</p>
                    </div>
                ...
                <div style={{ background: C.surface, border: `0.5px solid ${C.border}`, borderRadius: "14px", overflow: "hidden" }}>
                    <div style={{ padding: "18px 22px", borderBottom: `0.5px solid ${C.border}` }}>
                        ...
                        <div style={{ fontSize: "12px", color: C.muted, marginTop: "3px" }}>Daftar menu, harga, dan jumlah bahan terpeta</div>
                    </div>
                    <div style={{ padding: "16px 22px 0" }}>
                        <FilterTabs active={filterJenis} onChange={setFilterJenis} counts={counts} />
                    </div>

                    {filtered.length === 0 ? (
                        <EmptyState
                            title={filterJenis === "semua" ? "Belum ada menu" : `Belum ada menu ${filterJenis}`}
                            subtitle="Tambahkan menu dan pilih jenis catering saat mengisi form."
                        ...
                        <div style={{ overflowX: "auto" }}>
                            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px", minWidth: "860px" }}>
                                <thead>
                                    <tr>
                                        {["Foto", "Nama menu", "Jenis", "Kategori", "Harga", "Min. porsi", "Bahan", "Status", "Aksi"].map(h => (
                                    ...
                                    {filtered.map(item => (
                                        <tr key={item.id} style={{ borderBottom: `0.5px solid rgba(255,255,255,.04)` }}>
                                            <td style={{ padding: "10px 16px" }}>
                                                {item.image ? (
                                                    <img src={`/storage/${item.image}`} alt={item.name} style={{ width: "44px", height: "44px", objectFit: "cover", borderRadius: "8px", border: `0.5px solid ${C.border}` }} />
                                                ) : (
                                            ...
                                            <td style={{ padding: "12px 16px" }}>
                                                <JenisBadge jenis={item.jenis_catering || "Harian"} />
                                            </td>
                                            <td style={{ padding: "12px 16px" }}>
                                                {item.category ? (
                                            ...
                                            <td style={{ padding: "12px 16px", fontWeight: 700, color: C.text, whiteSpace: "nowrap" }}>{fmt(item.price)}</td>
                                            <td style={{ padding: "12px 16px", color: C.sub }}>{item.min_pax ? `${item.min_pax} porsi` : "—"}</td>
                                            <td style={{ padding: "12px 16px" }}>
                                                {item.ingredients_count > 0 ? (
                                                    <span style={{ display: "inline-flex", alignItems: "center", gap: "4px", fontSize: "11px", fontWeight: 600, padding: "3px 9px", borderRadius: "20px", background: "rgba(16,185,129,.10)", border: "0.5px solid rgba(16,185,129,.25)", color: "#34d399" }}>
                                                        <FlaskConical size={10} strokeWidth={2} />{item.ingredients_count} bahan
                                                    </span>
